...
- [x] **Mover a Supabase lo que vive en el navegador** (espejo user_kv, migración 0044) (si se salta este paso, esos datos no viajan entre dispositivos): menú de dopamina, rutinas guiadas, sesiones de Mente, estado de libros (leído / quiero leer), marca manual de ayuno, orden de las tarjetas y bloques de foco locales.
...
- [x] **Build limpio**: `cd app && npm run build` sin errores.
...
- [x] Variables de entorno (las mismas de `app/.env`):
  ...
  - `VITE_GEMINI_API_KEY`
- [x] Crear `app/vercel.json` con la regla SPA (todas las rutas sirven index.html):
  ```json
