...
- [ ] `[Nuevo]` Meta debería aparecer también en la página de su área asociada (si es de Finanzas, verse en Finanzas; si es de Trabajo, en Trabajo), no solo en Dirección. Rediseño de cómo cada página de área muestra sus metas conectadas: sesión aparte.
- [ ] `[Nuevo]` Rutinas guiadas: cada paso con su propio tiempo asignado por la usuaria (hasta 10 pasos), no solo texto libre. Rediseño de RutinasTab con un editor de pasos con minutos: sesión aparte.
- [ ] `[Nuevo]` Finanzas: automatizar transacciones recurrentes (como el traspaso a la tarjeta que termina en 4130) para solo aprobar en vez de categorizar cada vez. Ya existe una base (reglas por comercio, migración 0013); evaluar si extenderla alcanza o si hace falta un motor de reglas nuevo: sesión aparte.
- [ ] `[Nuevo]` Ciclo: agregar estado civil a la ficha; si está soltera, ocultar la sección de pareja y ajustar el copy para hablar en primera persona sola (hoy asume pareja siempre). Cambio acotado, pendiente de hacer.
- [ ] `[Nuevo]` Visión, visual board: fuente editable por nota (incluida una tipo caligrafía) y respetar el fondo transparente de imágenes PNG. El canvas hoy es de tamaño fijo (1200×760); en celular se ve chico y hay que deslizar mucho, rediseño responsive pendiente sin romper el arrastre: sesión aparte.
- [ ] `[Nuevo]` Ayuno: aclarar qué pasa si marcas "Acabo de comer" estando dentro de tu ventana de alimentación (hoy reinicia el contador, que es el comportamiento correcto pero puede no sentirse intuitivo). Revisar copy o agregar distinción entre "última comida" y "inicio de mi ventana de ayuno".
