...
- [x] `[Nuevo]` Idiomas, ola 4 (capturas de la usuaria, ~270 llaves por idioma): estrategia de llave-en-español para la capa de datos (si falta en un idioma, cae al español). Traducidos: las tarjetas de Energía Hoy (Agua, Proteína, Tu plato, energía, sueño, movimiento), los módulos y líneas de Revisión con su "abrir", el mes y eventos del Calendario, las tarjetas del Inicio (Tus áreas, Avances recientes, Menú de dopamina, citas y reconectar), el panel del pomodoro completo, los micros y botones de UnPaso, los estados de metas (En camino, Lograda...), los niveles de energía y las 4 fases del ciclo con sus consejos. Verificado en vivo en inglés. Queda el fondo editorial: los porqués e ideas de los 74 libros y las guías largas de cada página (candidatos a sesión propia o generación con IA revisada).
- [x] `[Nuevo]` Idiomas, olas 2 y 3 (170 llaves por idioma): los modales grandes traducidos (Nueva y Editar meta, Registrar avance, hábito, movimiento de Finanzas con Gasto/Ingreso/Transferencia, meta de ahorro, categoría), las tarjetas del Inicio (Tareas de hoy, ¿No sabes por dónde empezar?, Tu coach), el Despegue, el panel de temas, y los componentes compartidos hablando el idioma: Selector ("Choose"), CampoHora y el CALENDARIO COMPLETO por idioma (meses, días, "Pick a date", "Today", formato de fecha legible por idioma). Ola 3: la IA responde en el idioma elegido vía directiva central en generate() (verificado: el coach contestó en inglés fluido con datos reales). Queda de la ola 4: contenidos curados (biblioteca, guías, ciclo, acciones de vínculos) e interiores menores.
- [x] `[Nuevo]` Idiomas, fase 2, ola 1: la ESTRUCTURA de las 14 páginas habla los tres idiomas (106 llaves por idioma): cabeceras completas (categoría, título y subtítulo), todas las barras de pestañas (Dirección, Finanzas, Energía, Revisión, Visión, Hábitos, Mente, Movimiento, Aprendizaje), las estadísticas de Dirección, Trabajo y Finanzas, los botones principales (Nueva meta, Registrar jornada, Importar cartola...) y los Cargando. Verificado en vivo en inglés página por página. Quedan para las olas siguientes: interiores de tarjetas, modales, prompts de IA y contenidos curados.
- [x] `[Nuevo]` Idiomas, fase 1 (pedido de la usuaria): sistema propio de internacionalización (IdiomaProvider + diccionario plano con el español como fuente de verdad), selector 🌍 en Ajustes (Español, English, Português) que viaja entre dispositivos por el espejo de la nube, y la cáscara completa traducida: menú lateral con secciones y áreas, lema, miga del topbar y pantalla de login. Verificado en vivo en los tres idiomas. Fases siguientes: páginas módulo por módulo, prompts de IA en el idioma elegido, contenidos curados (biblioteca, guías, ciclo) y fechas con locale.
- [x] `[Nuevo]` MOTOR DIARIO (rediseño por frustración legítima de la usuaria): linkear un hábito a una meta ya NO sobreescribe la métrica de la meta (antes updateObjective pisaba los libros elegidos con habito_marcas). El vínculo ahora vive en el hábito (habits.meta_id, migración 0050), se puede poner y sacar también al EDITAR el hábito, y en las metas de libros exactos las marcas del hábito RELLENAN el tramo del libro en curso (hasta 90% del tramo; terminarlo lo completa). La tarjeta muestra "🔥 Motor diario: N días empujando la barra". Verificado con sus datos: 0% → 2% con una marca → 52% al terminar el primer libro.
- [x] `[Nuevo]` La meta de libros exactos ahora muestra sus libros con su estado en la tarjeta ("☐ ADHD 2.0 · ☐ Hábitos atómicos") y explica que avanza al marcar un libro como leído en la Biblioteca, mientras la lectura diaria vive en su hábito. Se acabó la confusión entre proceso (hábito de leer) y resultado (libros terminados).
- [x] `[Nuevo]` Bug de zona horaria en metas nuevas (cazado por la usuaria): la fecha de nacimiento de una meta se leía en UTC, así que una meta creada después de las 5 pm de Vancouver "nacía mañana" y no contaba los registros de su primer día. Ahora se mira en el día local. Verificado: su meta de lectura pasó de 0% a 2% con la marca de hoy.
- [x] `[Nuevo]` Migración 0049: el permiso de métricas (objectives_auto_metric_check) se reconstruye con las once métricas actuales; la usuaria había corrido la 0040 en su versión inicial y la base rechazaba momentos y libros. Los mensajes de error de crear y conectar metas ahora apuntan a la 0049 con claridad.
- [x] `[Nuevo]` Vía Psicología en la Biblioteca (pedido de la usuaria): 9 clásicos populares y efectivos sin duplicar otras vías (Kahneman, Flow, Gottlieb, Seligman, Ariely, Drive, Brené Brown, Dyer, Cialdini), cada uno con su porqué y sus tres ideas. La biblioteca queda en 74 libros, la IA bibliotecaria ya sugiere la vía nueva, y la migración 0048 amplía el check de libros propios.
- [x] `[Nuevo]` Bug cazado por la usuaria: la columna auto_ref de las metas era uuid y las conexiones nuevas guardan texto ("l:libros", "v:vía", "p:proyecto"). Migración 0047 la pasa a texto conservando todo, y la app avisa amablemente si falta.
