...
- [x] `[Nuevo]` La IA ahora viaja por el servidor: Edge Function "ia" (supabase/functions/ia/index.ts) que llama a Gemini con el secreto GEMINI_API_KEY y exige sesión; la app la usa primero y la llave local queda solo como respaldo de desarrollo. Pendiente de la usuaria: desplegar la función en el Dashboard, poner el secreto con una llave NUEVA (la actual está vencida, Google responde 401) y borrar VITE_GEMINI_API_KEY de Vercel.
