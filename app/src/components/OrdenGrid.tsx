...
export function OrdenGrid({ clave, bloques, lista = false, dosColumnas = false }: { clave: string; bloques: Bloque[]; lista?: boolean; dosColumnas?: boolean }) {
  ...
  return (
    <div className={"orden-grid" + (lista ? " orden-lista" : "") + (dosColumnas ? " orden-2col" : "")}>
      {orden.map((id) => (
        <div
          key={id}
