...
```
Telegram (Bot API); WhatsApp como canal premium futuro
        │  webhook inbound
        ▼
[1] Edge Function `wa-entrada`   → valida el secret token de Telegram, normaliza a evento único
...
[6] Capa de Tools NucleoOS ◄──────┘ escriben con service role ACOTADO al user_id del vínculo:
        ...
        ▼
[7] Salida Telegram                   → texto libre SIEMPRE (Telegram no tiene ventana ni templates)
        ▼
